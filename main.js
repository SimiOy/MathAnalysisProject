...
function draw(){
	...
	if(paused==0){ //running
		...
		if(mouseY > windowHeight*70/100 +30 && menuOn ==0 && allow && !mouseIsPressed)
			toggleMenu();
		...
		plot(intervals[0], intervals[intervals.length-1], totalFunction, 'green');
		if(gamePlaying)
		{
			animation.update();
			animation.draw();
...
{
	if(mouseIsPressed && mouseButton === LEFT && !keyIsDown(16) && ( mouseUpdates  || ( mouseUpdates==0 &&  mouseY<windowHeight*70/100) ) )
	{
